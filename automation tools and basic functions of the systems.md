...
Автоматизировать, потому что это будет быстрее и к тому же этот скрипт сможет использовать кто либо еще.
...
Виртуальным серверам не нужно очень много места, они сразу в компьютере. Они безопаснее по сравнению с физическими. Меньше ограничений по сравнению с физ. сервером. Недостатки: цена.
...
### Ответ 5:
Мне кажется, инфраструктуру изнутри потому что всегда нужно понимать что сильнее нагружает сервер и может быть стоило бы у чего нибудь ограничить мощность, а может наоборот прибавить, но и за доступностью стоит следить, потому что клиенту всегда должно было просто пользоваться нашим сервисом
...
* Да, вдруг что сломается после вашей настройки.
* Нет, т.к. он же уволился значит вряд ли здесь еще появится.
* Да, потому что не всегда обновление хорошо *ложится* на ПО
* Не обязательно, это всегда можно быстро исправить, но если изменение колоссальное, то, думаю, нужно
* Нет, это же копия, ничего не случится
* Да, в обновлениях тоже случаются баги и из за них может что то повредится.
